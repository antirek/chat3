 ...
 *     description: |
 *       Для всех диалогов пака, в которых пользователь является участником, выставляется
 *       `MessageStatus = read` и публикуется `dialog.messages.bulk_read`.
 *
 *       Обрабатываются только диалоги, где `userId` — member. Финальные счётчики — после counter-worker
 *       (асинхронно); поле `unreadCount` в ответе — optimistic.
 *
