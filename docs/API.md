...
Условные правила через `when` (для `required` и `unique`):
...
```json
{
  ...
  "when": {
    "key": "type",
    "op": "in",
    "value": ["phone", "telegram"]
  }
}
```

Семантика `when`:
- `op` в v1: `eq`, `in`, `ne`, `exists`;
- `exists` требует `value: true|false`;
- если `when` не задан, правило безусловное (как раньше);
- для `unique + when` уникальность проверяется только среди сущностей, где `when=true` (partial unique semantics).
...
Allowlist допустимых имён meta-ключей (одно правило на `entityType`, `indexId: allowed:keys`):
...
- Нет правила `mode: allowed` — любые ключи (как раньше).
- Есть allowlist — запись meta только для ключей из списка → иначе `META_KEY_NOT_ALLOWED`.
- `POST` с `keys: []` для allowlist → `INVALID_INDEX_SPEC`.
- Сужение allowlist при «лишних» ключах в Meta → `SCHEMA_CONFLICT_EXISTING_DATA` (+ `dryRun`).

Коды ошибок: `DUPLICATE_INDEX`, `INDEX_KEYS_REQUIRED`, `INDEX_CONFLICT_EXISTING_DATA`, `SCHEMA_CONFLICT_EXISTING_DATA`, `INDEX_DEFINITION_CONFLICT`, `INDEX_KEYS_NOT_IN_ALLOWLIST`, `META_KEY_NOT_ALLOWED`, `INVALID_INDEX_SPEC`.
