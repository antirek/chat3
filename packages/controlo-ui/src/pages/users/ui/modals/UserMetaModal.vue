<template>
  <BaseModal :is-open="isOpen" title="🏷️ Meta теги пользователя" max-width="600px" @close="close">
    <div class="meta-list">
      ...
      <table v-else>
        <thead><tr><th>Key</th><th>Value</th><th>Действия</th></tr></thead>
        <tbody>
          <tr v-for="(value, key) in metaTags" :key="key">
            <td><strong>{{ key }}</strong></td>
            <td>{{ JSON.stringify(value) }}</td>
            <td><BaseButton variant="danger" size="small" @click="deleteTag(String(key))">🗑️ Удалить</BaseButton></td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="meta-section">
      ...
      <div class="meta-tag-row">
        <input type="text" v-model="localMetaKey" placeholder="key (например: department)" />
        <input type="text" v-model="localMetaValue" placeholder="value (например: Sales)" />
