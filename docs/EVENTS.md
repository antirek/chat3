...
#### pack.changed
Изменение meta пака (`PUT/DELETE /meta/pack/...`).

**Routing Key:** `pack.changed.{tenantId}`

Секция `pack` с полным снимком `meta` после изменения. Update-worker создаёт **`update.dialog`** (`DialogUpdate`) с `uiTarget: dialogs.list` для участников всех диалогов пака (см. [PACK_CHANGED_DIALOG_UPDATE_PLAN.md](./PACK_CHANGED_DIALOG_UPDATE_PLAN.md)).
