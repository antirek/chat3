...
```javascript
{
  ...
  entityId: "dlg_...",
  eventId: "evt_...",
  sourceEventType: "message.create",  // доменный Event (Event.eventType)
  updateType: "update.dialog",        // update.message | update.dialog | update.user
  data: {
    context: {
      version: 4,
...
| Поле | Смысл |
|------|--------|
| `eventId` | ссылка на Event |
| `sourceEventType` | тип доменного события |
| `updateType` | семейство Update (`update.*`) |
| `data.context.uiTarget` | куда применить в UI |

Полный план: [UPDATE_TYPE_NAMING_PLAN.md](./UPDATE_TYPE_NAMING_PLAN.md).
