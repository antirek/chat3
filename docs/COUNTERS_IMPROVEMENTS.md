...
| 6 | Кеширование типов пользователей | Важно | ✅ Частично | Загружаются одним запросом, но нет кеширования между запросами |
...
| 8 | Поиск Event в MessageStatus middleware | Производительность | ⏳ Не реализовано | Требуется передавать `sourceEventId` напрямую |
