...
describe('messageController.createMessage - unread handling', () => {
  ...
  test('should create user.stats.update event when dialog becomes unread (unreadCount 0 -> 1)', async () => {
    // Убеждаемся, что у bob unreadCount = 0 (диалог прочитан)
    await DialogMember.updateOne(
      { tenantId, dialogId: dialog.dialogId, userId: 'bob' },
      { $set: { unreadCount: 0, isActive: true } }
    );
    ...
    // Проверяем, что unreadCount увеличился
    const bobMember = await DialogMember.findOne({ tenantId, dialogId: dialog.dialogId, userId: 'bob' }).lean();
    expect(bobMember?.unreadCount).toBe(1);
    ...
    if (userStatsEvent) {
      expect(userStatsEvent.data.user.userId).toBe('bob');
      expect(userStatsEvent.data.user.stats.unreadDialogsCount).toBeGreaterThan(0);
    }
  });

  test('should not create user.stats.update event when dialog was already unread (unreadCount >0 -> >0)', async () => {
    // Устанавливаем unreadCount > 0 (диалог уже непрочитан)
    await DialogMember.updateOne(
      { tenantId, dialogId: dialog.dialogId, userId: 'bob' },
    ...
    // Проверяем, что unreadCount увеличился
    const bobMember = await DialogMember.findOne({ tenantId, dialogId: dialog.dialogId, userId: 'bob' }).lean();
    expect(bobMember?.unreadCount).toBe(3);

    // Проверяем, что событие user.stats.update НЕ создалось (статус не изменился)
    const userStatsEvent = await Event.findOne({
      tenantId,
      eventType: 'user.stats.update',
      entityId: 'bob',
