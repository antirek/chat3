...
### Этап 5: Обновление тестового интерфейса API (api-test)

1. Обновить `src/apps/api-test/public/api-test-user-dialogs.html`:
   - **Добавить третью колонку "Топики"** в разделе "Пользователи - Диалоги - Сообщения"
     - Изменить структуру: вместо трех колонок (Пользователи, Диалоги, Сообщения/Участники) сделать три колонки (Пользователи, Диалоги, Сообщения/Участники/Топики)
   - **В колонке "Диалоги" в таблице диалогов:**
     - В колонке "Просмотр" добавить кнопку "Топики" рядом с существующими кнопками
     - При нажатии на кнопку "Топики" открыть список топиков диалога в третьей колонке
     - Добавить примеры фильтров в секцию фильтров:
       - Фильтр по `topicId`: `topicId,eq,{topicId}`
       - Фильтр по мета-тегам топика: `topic.meta.{param},eq,{value}`
       - Примеры: `topicId,eq,topic_abc123`, `topic.meta.category,eq,general`
   - **В колонке "Топики" (третья колонка):**
     - Вывести таблицу с топиками диалога
     - Колонки таблицы:
       - `topicId` - ID топика
       - `Meta` - мета-теги топика (отобразить в читаемом формате, например JSON или список ключ-значение)
     - Загружать топики через `GET /api/dialogs/{dialogId}/topics`
     - При клике на топик можно показать детальную информацию (мета-теги, количество сообщений и т.д.)
   - **Интеграция с существующим функционалом:**
     - Сохранить возможность просмотра сообщений в третьей колонке
     - Добавить переключение между режимами "Сообщения" и "Топики" в третьей колонке
...
6. Обновить тестовый интерфейс API (api-test) для работы с топиками
7. Создать скрипт миграции для `DialogStats`
8. Написать тесты
9. Запустить миграцию для существующих диалогов
