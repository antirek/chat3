...
export function buildConfigJsContent(config: ControloConfig): string {
  const publicPath = normalizePublicPath(config.CONTROLO_PUBLIC_PATH);
...
window.CHAT3_CONFIG = {
    ...
    CONTROLO_PUBLIC_PATH: ${JSON.stringify(publicPath)},
...
/** Vite `base: './'` emits relative asset URLs; rewrite to absolute under public path. */
const ROOT_RELATIVE_ATTR_RE = /(\s(?:src|href)=["'])\.\/(config\.js|vite\.svg|assets\/|src\/)/g;
...
export function rewriteSpaIndexHtml(html: string, controloPublicPath: string): string {
  ...
  return html
    .replace(ROOT_ABSOLUTE_ATTR_RE, `$1${publicPath}/$2`)
    .replace(ROOT_RELATIVE_ATTR_RE, `$1${publicPath}/$2`);
}
