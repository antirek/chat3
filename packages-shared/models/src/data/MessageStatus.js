...
import { updateStatusCount, updateUnreadCount, finalizeCounterUpdateContext } from '@chat3/utils/counterUtils.js';
    ...
    if (doc.isNew) {
        try {
            ...
            try {
                ...
                await updateStatusCount(doc.tenantId, doc.messageId, doc.status, 1, // delta
                sourceEventType, doc.userId, 'user');
                ...
                if (oldStatus !== 'read' && doc.status === 'read') {
                    ...
                    if (doc.dialogId) {
                        ...
                        await updateUnreadCount(doc.tenantId, doc.userId, doc.dialogId, -1, // delta (уменьшаем при прочтении)
                        sourceEventType, sourceEventId, doc.messageId, doc.userId, 'user', topicId // topicId для обновления счетчиков топика
                        );
                    }
                }
            }
...
messageStatusSchema.post('deleteOne', async function (doc) {
    try {
        ...
        await updateStatusCount(doc.tenantId, doc.messageId, doc.status, -1, // delta
        'message.status.update', doc.userId, 'user');
        console.log(`✅ MessageStatusStats updated: ${doc.tenantId}/${doc.messageId}/${doc.status} (-1)`);
        ...
        if (doc.status === 'read' && doc.dialogId) {
            ...
            await updateUnreadCount(doc.tenantId, doc.userId, doc.dialogId, 1, // delta (увеличиваем при удалении статуса 'read')
            'message.status.update', null, // sourceEventId
            doc.messageId, doc.userId, 'user', topicId);
