...
export function formatTimestamp(
  ...
  format: 'full' | 'date' | 'time' | 'datetime' | 'iso' | 'relative' = 'full',
  locale: string = 'ru-RU'
): string {
  let date: Date;
  let microseconds = 0;
  ...
  if (typeof timestamp === 'number') {
    ...
    if (match) {
      microseconds = parseInt(match[1], 10);
    }
 ...
 */
export function formatTimestampFixed(timestamp: number, decimals: number = 6): string {
 ...
 */
export function getMongoTimestamp(): { date: Date; microseconds: number } {
  const hrTime = process.hrtime.bigint();
