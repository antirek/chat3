...
const dialogMemberController = {
  ...
  async addDialogMember(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    try {
      ...
      const { userId, type, name } = req.body as { userId?: string; type?: string; name?: string };
  ...
  async getDialogMembers(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    try {
      ...
      const page = parseInt(String(pageParam || '1'), 10) || 1;
      const limit = parseInt(String(limitParam || '10'), 10) || 10;
      ...
      if (filter) {
        try {
          ...
          parsedFilters = parseFilters(String(filter));
      ...
      if (sortParam) {
        const sortParamStr = String(sortParam);
        // Проверяем формат (field,direction)
        const sortMatch = sortParamStr.match(/^\(([^,]+),([^)]+)\)$/);
        if (sortMatch) {
          ...
          sortField = sortParamStr;
          const sortDirStr = sortDirection ? String(sortDirection) : 'desc';
          sortDir = sortDirStr === 'asc' ? 1 : -1;
        }
      } else if (sortDirection) {
        const sortDirStr = String(sortDirection);
        sortDir = sortDirStr === 'asc' ? 1 : -1;
      }
  ...
  async removeDialogMember(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  async setUnreadCount(req: AuthenticatedRequest, res: Response): Promise<void> {
...
function extractUserIdFromEntityId(entityId: any): string | null {
...
async function findMemberUserIdsByMeta(tenantId: string, dialogId: string, metaFilters: any): Promise<string[]> {
  ...
  for (const [key, condition] of Object.entries(metaFilters)) {
    ...
    if (isNegativeOperator) {
      ...
      if (Array.isArray(conditionAny.$nin)) {
        const excludeSet = new Set(conditionAny.$nin);
      ...
      } else if (conditionAny.$ne !== undefined) {
        candidateUserIds = allWithKey
          .filter(record => record.value !== conditionAny.$ne)
          .map(record => extractUserIdFromEntityId(record.entityId))
          .filter(Boolean) as string[];
      }
    } else {
