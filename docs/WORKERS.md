...
Dialog Read Worker обрабатывает задачи `DialogReadTask`, массово проставляя статус `read` всем сообщениям диалога до указанного `readUntil`.  
Задачи создаются API после вызова `PATCH /api/dialogs/:dialogId/members/:userId/unread`, когда счётчик `unreadCount` сбрасывается и требуется привести `MessageStatus` в консистентное состояние.

**Файл:** `src/workers/dialogReadWorker.js`  
**Запуск:** `./start-dialog-read-worker.sh` или `npm run start:readWorker`

### Основные шаги

1. Находит задачу со статусом `pending`, помечает как `running`.
2. Получает сообщения диалога пачками (`DIALOG_READ_BATCH_SIZE`) до `readUntil`.
3. Для каждой пачки выполняет `bulkWrite` по `MessageStatus` (upsert со статусом `read`), пропуская сообщения, отправленные самим пользователем.
4. Сохраняет прогресс (`lastProcessedAt`, `processedCount`) и после завершения ставит статус `completed`.

### Конфигурация

```bash
DIALOG_READ_BATCH_SIZE=200        # размер батча при обработке сообщений
DIALOG_READ_BATCH_SLEEP_MS=0      # пауза между батчами
DIALOG_READ_TASK_POLL_MS=2000     # интервал опроса новых задач
```

### Мониторинг

```javascript
db.dialogreadtasks.find().sort({ createdAt: -1 }).limit(5)
db.dialogreadtasks.find({ status: { $in: ['pending', 'running', 'failed'] } })
```

Логи воркера:
- `🧹 Running dialog read task ...` — начало обработки
- `✅ Task ... completed` — успех
- `❌ Task ... failed` — ошибка (см. поле `error` в документе)

### Остановка

Worker реагирует на SIGINT/SIGTERM, завершает текущую пачку и корректно останавливается.
