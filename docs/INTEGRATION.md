...
| Users | `POST /users` | Создание пользователя. Можно указать `type` (опционально, по умолчанию `user`). | write |
| Users | `PUT /users/:userId` | Обновление пользователя. Можно изменить `name` и `type`. | write |
