...
const CONTROL_APP_URL = resolveControlAppUrl(process.env.CONTROL_APP_URL);
...
app.use('/api-docs', swaggerUi.serve, (req, res, next) => {
  const swaggerServerUrl = resolveSwaggerServerUrl(
    process.env.CONTROL_APP_URL,
...
if (existsSync(controloUiDistPath)) {
  ...
  app.get('*', (req, res, next) => {
    ...
    if (!cachedIndexHtml) {
      cachedIndexHtml = readFileSync(join(controloUiDistPath, 'index.html'), 'utf-8');
    }

    const html = rewriteSpaIndexHtml(cachedIndexHtml, CONTROLO_PUBLIC_PATH);
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      if (CONTROLO_PUBLIC_PATH) {
        console.log(`📂 Public path prefix: ${CONTROLO_PUBLIC_PATH}`);
      }
