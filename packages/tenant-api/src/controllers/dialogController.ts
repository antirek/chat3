...
export const dialogController = {
  ...
  async getAll(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    try {
        ...
        try {
          ...
          if (Object.keys(metaFilters).length > 0) {
            ...
            for (const [key, condition] of Object.entries(metaFilters)) {
              let foundDialogIds: string[];
              
              // Обработка негативных операторов ($ne, $nin) требует специальной логики
              const conditionAny = condition as any;
              const isNegativeOperator = typeof condition === 'object' && (conditionAny.$ne !== undefined || conditionAny.$nin !== undefined);
              ...
              if (isNegativeOperator) {
                ...
                if (conditionAny.$ne !== undefined) {
                  // $ne: не равно конкретному значению
                  foundDialogIds = allWithKey
                    .filter(m => m.value !== conditionAny.$ne)
                    .map(m => m.entityId.toString());
                } else if (conditionAny.$nin !== undefined) {
                  // $nin: не в массиве значений
                  foundDialogIds = allWithKey
                    .filter(m => !conditionAny.$nin.includes(m.value))
                    .map(m => m.entityId.toString());
                }
              } else {
      ...
      const sortField = req.query.sort ? String(req.query.sort) : undefined;
      ...
      if (isCreatedAtSort) {
        ...
        let baseQuery: any = { tenantId: req.tenantId! };
        ...
        dialogsWithValidMembers.sort((a: any, b: any) => {
        ...
        dialogs = await Dialog.aggregate(pipeline as any) as any;
      } else {
        // Обычный запрос
        dialogs = await Dialog.find(query)
          .skip(skip)
  ...
  async create(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    try {
      const { members, meta: metaPayload } = req.body as { members?: any[]; meta?: any };
