...
| `dialog.member.add` | `dialogController.create`<br>`dialogMemberController.addDialogMember`<br>`userPackController.joinPack` | `UserDialogStats.unreadCount` (пересчёт)<br>`UserDialogUnreadBySenderType`<br>`UserStats.dialogCount` (+1)<br>`UserStats.totalUnreadCount`, `unreadDialogsCount` (пересчёт)<br>`UserPackUnreadBySenderType` (если диалог в паке) | **counter-worker** (`recalculateSlice` + `updateUserStatsDialogCount`) | Unread только для сообщений с `createdAt >= DialogMember.createdAt` (история до join не в бейдже). Push: `UserStatsUpdate` |
| `dialog.member.remove` | `dialogMemberController.removeDialogMember`<br>`userPackController.leavePack` | `UserDialogUnreadBySenderType` (удаление per-dialog)<br>`UserStats.dialogCount` (-1)<br>`UserStats.totalUnreadCount`, `unreadDialogsCount` (пересчёт)<br>`UserPackUnreadBySenderType` (пересчёт по пакам диалога)<br>`DialogStats.memberCount` | **counter-worker** (`recalculateSlice` + `updateUserStatsDialogCount`) | API синхронно удаляет `DialogMember`, `UserDialogStats`, `UserDialogActivity`, `UserDialogUnreadBySenderType`; агрегаты — только worker. После `markAllReadForAllUsers` remove из другого диалога пака **не восстанавливает** contact unread (см. `markAllReadThenMemberRemove.integration.test.js`). Push: `UserStatsUpdate` |
