 ...
 */
export function validateGetUserDialogMessagesResponse(response: ResponseData): ValidationResult {
 ...
 * - statuses отсутствует в корне объекта сообщения
 */
export function validateGetUserDialogMessageResponse(response: ResponseData): ValidationResult {
  if (!response || !response.data) {
    return { valid: false, error: 'Response must have data field' };
  }
